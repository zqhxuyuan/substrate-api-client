...
impl Metadata {
		...
		S: ToString,
	{
		...
		self.modules.get(&name).ok_or(MetadataError::ModuleNotFound(name))
	}
	...
	{
		let name = name.to_string();
		self.modules_with_calls.get(&name).ok_or(MetadataError::ModuleNotFound(name))
	}
	...
	{
		let name = name.to_string();
		self.modules_with_events.get(&name).ok_or(MetadataError::ModuleNotFound(name))
	}
	...
	{
		let name = name.to_string();
		self.modules_with_errors.get(&name).ok_or(MetadataError::ModuleNotFound(name))
	}
	...
	pub fn storage_value_key(
		...
		Ok(self.module(storage_prefix)?.storage(storage_key_name)?.get_value()?.key())
	}
	...
	pub fn storage_map_key_prefix(
		...
		self.module(storage_prefix)?.storage(storage_key_name)?.get_map_prefix()
	}
...
impl ModuleWithEvents {
	...
	pub fn event(&self, index: u8) -> Result<&ModuleEventMetadata, MetadataError> {
		self.events.get(&index).ok_or(MetadataError::EventNotFound(index))
	}
...
impl ModuleWithErrors {
	...
	pub fn error(&self, index: u8) -> Result<&String, MetadataError> {
		self.errors.get(&index).ok_or(MetadataError::ErrorNotFound(index))
	}

	pub fn print(&self) {
		println!("----------------- Errors for Module: {} -----------------\n", self.name());
...
impl TryFrom<RuntimeMetadataPrefixed> for Metadata {
	...
	fn try_from(metadata: RuntimeMetadataPrefixed) -> Result<Self, Self::Error> {
		...
		Ok(Metadata { modules, modules_with_calls, modules_with_events, modules_with_errors })
	}
}
