...
use crate::{
	...
	utils, ApiClientError,
};

pub use client::WsRpcClient;
pub use events::{EventsDecoder, EventsError, RawEvent, RuntimeEvent};
...
fn parse_status(msg: &str) -> RpcResult<(XtStatus, Option<String>)> {
	...
	match value["error"].as_object() {
		Some(obj) => {
			let error = obj.get("message").map_or_else(|| "", |e| e.as_str().unwrap());
...
mod tests {
	...
	fn extrinsic_status_parsed_correctly() {
		...
		assert_eq!(
			parse_status(msg).map_err(extract_extrinsic_error_msg).unwrap_err(),
