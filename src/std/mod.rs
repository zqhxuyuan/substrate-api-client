...
pub use crate::{std::rpc::XtStatus, utils::FromHexString};
...
use crate::{extrinsic, rpc::json_req, AccountData, AccountInfo, Balance, Hash};
	...
	Client: RpcClient,
{
	pub fn new(client: Client) -> ApiResult<Self> {
		...
		Ok(Self { signer: None, genesis_hash, metadata, runtime_version, client })
	}
...
fn inclusion_fee_with_balance(
	...
	Ok(InclusionFee {
		...
		len_fee: inclusion_fee.base_fee.try_into().map_err(|_| ApiClientError::TryFromIntError)?,
